...
// Conversión tables removed due sfcc jsObject quota limit
//var cptable = require('./cpexcel');
//var current_cptable = cptable[current_codepage];
        ...
        function write_num_flt(type, fmt, val) {
            ...
            if ((r = fmt.match(/^#,##0\.([#0]*0)$/)) !== null) {
                return val < 0
                    ...
                    : commaify('' + Math.floor(val)) + '.' + pad0(dec(val, r[1].length), r[1].length);
            }
...
function writextag(f, g, h) {
    return (
        ...
        (isval(g) ? (g.match(wtregex) ? ' xml:space="preserve"' : '') + '>' + g + '</' + f : '/') +
        '>'
    );
}
...
function parse_comments_xml(data, opts) {
    ...
    var authors = [];
    var commentList = [];
    data.match(/<(?:\w+:)?authors>([^\u2603]*)<\/(?:\w+:)?authors>/)[1]
        .split(/<\/\w*:?author>/)
        .forEach(function(x) {
            if (x === '' || x.trim() === '') return;
            authors.push(x.match(/<(?:\w+:)?author[^>]*>(.*)/)[1]);
        });
    (data.match(/<(?:\w+:)?commentList>([^\u2603]*)<\/(?:\w+:)?commentList>/) || ['', ''])[1]
        .split(/<\/\w*:?comment>/)
...
var WBPropsDef = [
    ...
    ['codeName', ''],
    ['date1904', '0'],
    ['dateCompatibility', '1'], //['defaultThemeVersion', '0'],
    ['filterPrivacy', '0'],
    ...
    ['showInkAnnotation', '1'],
    ['showObjects', 'all'],
    ['showPivotChartFilter', '0'] //['updateLinks', 'userSet']
];
