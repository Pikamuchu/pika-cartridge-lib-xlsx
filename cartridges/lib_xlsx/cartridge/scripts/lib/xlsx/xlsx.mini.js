...
function make_xlsx_lib(XLSX) {
    ...
    var make_ssf = function make_ssf(SSF) {
        ...
        function datenum_local(v, date1904) {
            ...
            return (
                (epoch - (dnthresh + (v.getTimezoneOffset() - basedate.getTimezoneOffset()) * 60000)) /
                (24 * 60 * 60 * 1000)
            );
        }
    ...
    */
        ...
        function get_fs() {
            return fs || (fs = require('*/cartridge/scripts/lib/fs/index'));
        }
    ...
    if (typeof require !== 'undefined')
        try {
            _fs = require('*/cartridge/scripts/lib/fs/index');
        } catch (e) {}
    ...
    function write_dl(fname, payload, enc) {
        ...
        if (typeof Blob !== 'undefined') {
            ...
            if (
                ...
                if (
                    (typeof chrome === 'undefined' ? 'undefined' : _typeof(chrome)) === 'object' &&
    ...
    function zip_add_file(zip, path, content) {
        if (zip.FullPaths)
            CFB.utils.cfb_add(
                zip,
                path,
                typeof content == 'string' ? (has_buf ? Buffer_from(content) : s2a(utf8write(content))) : content
            );
    ...
    var htmldecode = (function() {
        ...
        return function htmldecode(str) {
            var o = str // Remove new lines and spaces from start of content
                ...
                .replace(/>\s+/g, '>')
                .replace(/\s+</g, '<') // Replace remaining new lines and spaces with space
        ...
        function dbf_to_aoa(buf, opts) {
            ...
            if (ft != 0x02)
                if (d.l < d.length && d[d.l++] != 0x1a)
                    throw new Error(
                        'DBF EOF Marker missing ' + (d.l - 1) + ' of ' + d.length + ' ' + d[d.l - 1].toString(16)
                    );
        ...
        function sheet_to_dbf(ws, opts) {
            ...
            for (i = 0; i < headers.length; ++i) {
                ...
                if (typeof headers[i] !== 'string')
                    throw new Error(
                        'DBF Invalid column name ' + headers[i] + ' |' + _typeof(headers[i]) + '|'
                    );
    ...
    var SYLK = (function() {
        ...
        function sheet_to_sylk(ws, opts) {
            ...
            preamble.push(
                'B;Y' + (r.e.r - r.s.r + 1) + ';X' + (r.e.c - r.s.c + 1) + ';D' + [r.s.c, r.s.r, r.e.c, r.e.r].join(' ')
            );
    ...
    function parse_si(x, opts) {
        ...
        /* 18.4.4 r CT_RElt (Rich Text Run) */
            ...
            z.r = utf8read(x);
            z.t = unescapexml(
                utf8read((x.replace(sirphregex, '').match(sitregex) || []).join('').replace(tagregex, ''))
            );
    ...
    */
    ...
    var WBViewDef = [
        ...
        ['tabRatio', 600, 'int'],
        ['visibility', 'visible'] //window{Height,Width}, {x,y}Window
    ];
  ...
  */
    ...
    function get_get_computed_style_function(element) {
        ...
        if (
            ...
            return element.ownerDocument.defaultView.getComputedStyle; // If it is not available, try to get one from the global namespace
